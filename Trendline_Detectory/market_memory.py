...
class MarketMemory:
    ...
    def _generate_context_narrative(
        ...
        # Regime
        if regime:
            duration = (datetime.utcnow() - datetime.fromisoformat(regime['start_time'])).total_seconds() / 3600
    ...
    def get_memory_stats(self) -> Dict:
        ...
        if oldest:
            days = (datetime.utcnow() - datetime.fromisoformat(oldest)).total_seconds() / 86400
        else:
            days = 0
...
def print_market_context(memory: MarketMemory, lookback_hours: float = 24):
    ...
    if context['active_range_period']:
        ...
        duration = (datetime.utcnow() - datetime.fromisoformat(rng['start_time'])).total_seconds() / 3600
