...
"""
    ...
    if old_code in content:
        ...
        print("Could not find the exact pattern to replace. Here's a character-by-character comparison:")
        # Try a more flexible matching approach
        lines = content.split('\n')
        found = False
        for i, line in enumerate(lines):
            if 'if not htf_swing_highs.empty and not htf_swing_lows.empty:' in line:
                print(f"Found potential starting point at line {i+1}")
                # Print the next 15 lines to see what's there
