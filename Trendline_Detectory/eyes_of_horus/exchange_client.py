...
class BybitClient:
    ...
    async def set_leverage(self, symbol: str, leverage: int) -> bool:
        ...
        if response and response.get('retCode') == 0:
            ...
            if "leverage not modified" in ret_msg:
                ...
                logger.error(f"Failed to set leverage for {symbol}: {ret_msg}")
                return False
    ...
    async def get_available_balance(self) -> float:
        ...
        for coin in coins:
            if coin.get('coin') == 'USDT':
                try:
                    # Try multiple fields for available balance, preferring the most liquid ones
                    ...
                    # Use the highest non-zero value as the most likely available balance
                    balance = max(available_float, wallet_float, equity_float)
                    if balance > 0:
                        logger.info(f"Found USDT balance: ${balance:.4f}")
                        return balance
    ...
    async def _calculate_and_validate_qty(self, symbol: str, current_price: float, allocated_balance: float) -> str:
        ...
        # 3. Calculate desired quantity with leverage
        ...
        margin_to_use = allocated_balance * percentage_of_balance
        desired_usd_value = margin_to_use * leverage
        raw_qty = desired_usd_value / current_price

        logger.info(f"[SIZE_CALC] Allocated Balance: ${allocated_balance:.2f}, Margin: ${margin_to_use:.2f}, Leverage: {leverage}x, USD Value: ${desired_usd_value:.2f}, Raw Qty: {raw_qty:.4f}")
