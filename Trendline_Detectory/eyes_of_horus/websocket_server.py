        ...
        try:
            async for message in websocket:
                try:
                    ...
                    direction = data.get('decision', {}).get('direction')
                    confidence = data.get('decision', {}).get('confidence')
                    symbol = data.get('symbol', 'UNKNOWN')
                    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

                    logger.info(f"[{timestamp}] Received signal from {client_address}: {direction} {symbol}, confidence: {confidence}")
                    ...
                    if 'entry_zone' in decision:
                        ...
                        take_profits = decision.get('take_profits', [])
                        logger.debug(f"  - Entry zone: {entry_zone}, Stop Loss: {stop_loss}, Take Profits: {take_profits}")
