...
def fix_structural_analyzer():
    ...
    # Build the exact original block with proper indentation and spacing
    old_code = """            if not htf_swing_highs.empty and not htf_swing_lows.empty:
                ...
                if high_level > low_level:
                    ...
                    # This is the problematic case - set to None to indicate invalid HTF context
                    htf_context["htf_swing_high"] = None
                    htf_context["htf_swing_low"] = None
                    htf_context["fib_levels"] = None
                    htf_context["price_location"] = "Invalid HTF Context - No Valid Swing Range"
                    print(f"[ERROR] Invalid HTF context: high ({{high_level}}) <= low ({{low_level}})")
"""
    
    new_code = """            if not htf_swing_highs.empty and not htf_swing_lows.empty:
...
"""
    ...
    if old_code in content:
        ...
        print("Original code block was replaced with improved ranging market handling.")
    else:
        print("Could not find the exact pattern to replace.")
        print("Let's try a more flexible approach by finding the starting pattern...")
        
        # Find the starting pattern and replace from there
        start_pattern = "if not htf_swing_highs.empty and not htf_swing_lows.empty:"
        ...
        if start_idx != -1:
            ...
            if start_line_num != -1:
                ...
                print(f"Found block from line {start_line_num+1} to {end_line_num+1}")
                ...
                print("Flexible replacement applied successfully!")
        else:
            print("Could not locate the pattern to replace.")
