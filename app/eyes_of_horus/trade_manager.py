...
from eyes_of_horus.database import get_db_session, init_db
from eyes_of_horus.models import ManagedTrade
from eyes_of_horus.exchange_client import BybitClient
from eyes_of_horus.websocket_server import WebSocketServer
from eyes_of_horus.technical_analysis import calculate_rsi, calculate_atr
from eyes_of_horus.filters import KalmanFilter
...
class TradeManager:
    ...
    def __init__(self, host: str = "localhost", port: int = 8765):
        ...
        self.max_concurrent_symbols = 3 # Back to 3 to allow proper position sizing per symbol
    ...
    def _load_state(self):
        ...
        try:
            ...
            if self.cooldown_until and self.cooldown_until > get_utc_now():
                 logger.info(f"  - Cooldown is active until: {self.cooldown_until}")
            if self.regime_filter_until and self.regime_filter_until > get_utc_now():
    ...
    def _activate_cooldown(self):
        ...
        self.cooldown_until = get_utc_now() + timedelta(hours=1)
    ...
    def _activate_regime_filter(self):
        ...
        self.regime_filter_until = get_utc_now() + timedelta(hours=3)
    ...
    async def _sync_with_bybit_positions(self):
        """Sync Aegis state with actual Bybit open positions to update memory."""
        try:
            # Get actual open positions from Bybit
            exchange_positions = await self.bybit_client.get_all_open_positions()

            logger.info(f"[SYNC] Found {len(exchange_positions)} positions on Bybit")

            # Get current managed trades from database with a fresh session
            # We need to handle this in a single session context to avoid detached instance errors
            with get_db_session() as db:
                managed_trades_from_db = db.query(ManagedTrade).filter(ManagedTrade.status == 'active').all()

                # Create sets of symbols for comparison
                bybit_symbols = set()
                for pos in exchange_positions:
                    if float(pos.get('size', 0)) > 0:  # Only consider positions with actual size
                        bybit_symbols.add(pos['symbol'])

                db_symbols = {trade.symbol for trade in managed_trades_from_db}

                logger.info(f"[SYNC] Bybit reports: {bybit_symbols}")
                logger.info(f"[SYNC] DB shows active: {db_symbols}")

                # For positions that exist in DB but not on Bybit - close them in DB
                updated_trades = []
                for trade in managed_trades_from_db:
                    if trade.symbol not in bybit_symbols and trade.status == 'active':
                        logger.warning(f"[SYNC] Position for {trade.symbol} exists in DB but not on Bybit. Updating to closed.")
                        trade.status = 'closed'
                        trade.exit_reason = 'SYNC_EXTERNAL_CLOSE'
                        trade.exit_price = await self.bybit_client.get_current_price(trade.symbol)
                        if trade.direction == 'LONG':
                            trade.pnl = (trade.exit_price - trade.entry_price) * trade.qty
                        else:
                            trade.pnl = (trade.entry_price - trade.exit_price) * trade.qty
                        updated_trades.append(trade.symbol)

                # Commit changes to database while in the same session
                db.commit()

                # For positions that exist on Bybit but not in DB - import them
                db_managed_symbols = {trade.symbol for trade in managed_trades_from_db}
                for pos in exchange_positions:
                    symbol = pos['symbol']
                    if float(pos.get('size', 0)) > 0 and symbol not in db_managed_symbols:
                        logger.info(f"[SYNC] Found unmanaged position for {symbol} on Bybit. Creating managed record.")
                        # Import this position as a managed trade
                        await self._create_recovered_trade_record(pos, db_session=db)

            # Update in-memory sessions to ensure they reflect the closed positions
            # This should be done after the database session is closed to avoid detached instance errors
            for symbol in updated_trades:
                # Remove from managed sessions if present
                sessions_to_remove = []
                for session_id, session_data in self.managed_sessions.items():
                    # Check if this session needs to be removed
                    # First, try getting the trade object from the session data
                    session_trade = session_data.get('trade')
                    should_remove = False

                    # Check if the trade object exists in session data and matches the symbol to be closed
                    if session_trade:
                        # Check if it's a valid object with symbol attribute
                        if hasattr(session_trade, 'symbol') and hasattr(session_trade, 'status'):
                            # Check if session trade is active and matches the symbol being closed
                            # We need to handle potential detached instance error carefully
                            try:
                                if session_trade.symbol == symbol and session_trade.status == 'active':
                                    should_remove = True
                            except Exception:
                                # If we get an error accessing this detached instance,
                                # let's check if there's other metadata we can use
                                if session_data.get('symbol') == symbol and session_data.get('status') == 'active':
                                    should_remove = True
                        else:
                            # Alternative: Check if trade_id or symbol is accessible through session directly
                            if session_data.get('symbol') == symbol and session_data.get('status') == 'active':
                                should_remove = True

                    # If the main trade object wasn't valid, check session metadata
                    elif session_data.get('symbol') == symbol and session_data.get('status') == 'active':
                        should_remove = True

                    if should_remove:
                        logger.info(f"[SYNC] Removing {symbol} from managed sessions (in-memory cache)")
                ...
                logger.info(f"[SYNC] Updated {symbol} to closed status in database and cleared from memory.")

            # Get the final active symbols from a new session to avoid detached errors
            with get_db_session() as db:
                active_managed_trades_from_db = db.query(ManagedTrade).filter(ManagedTrade.status == 'active').all()
                current_managed_symbols = {trade.symbol for trade in active_managed_trades_from_db}

            logger.info(f"[SYNC] After sync, active symbols: {current_managed_symbols}")

        except Exception as e:
            logger.error(f"[SYNC] Error during Bybit position sync: {e}", exc_info=True)

    async def handle_new_signal(self, signal_data: dict):
        """Callback for the WebSocket server to process a new limit order signal."""

        # First, sync with Bybit to ensure we have accurate position count
        await self._sync_with_bybit_positions()
        ...
        # We need a temporary session to safely access trade data
        with get_db_session() as temp_db:
            for session in self.managed_sessions.values():
                ...
                if trade_from_db and trade_from_db.symbol == symbol and trade_from_db.status == 'active':
                    logger.warning(f"[{trade_id}] Received signal for {symbol}, but a position is already active. Ignoring signal.")
                    return
        ...
        try:
            ...
            # Override the stop loss with the fixed 0.3% value to prevent overly tight stops from liquidity analysis
            # This ensures consistent, predictable stop losses that won't get triggered too quickly
            entry_price = decision['entry_zone'][0]
            direction = decision['direction']

            # Calculate fixed 0.3% stop loss
            sl_distance = entry_price * 0.003  # 0.3%
            if direction == 'LONG':
                fixed_stop_loss = entry_price * 0.997  # 0.3% below entry
            else:  # SHORT
                fixed_stop_loss = entry_price * 1.003  # 0.3% above entry

            # Update the decision with the fixed stop loss
            decision['stop_loss'] = fixed_stop_loss
            logger.info(f"[{trade_id}] Overrode signal stop loss with fixed 0.3% stop loss: ${fixed_stop_loss:.4f}")
            ...
            # --- CORRECTED DYNAMIC BALANCE ALLOCATION LOGIC ---
            # Calculate allocation as a fixed percentage of account divided by max symbols
            # This ensures consistent position sizing throughout the lifecycle
            # For example: 95% of account balance / 3 max symbols = consistent ~31% per position
            total_account_balance = await self.bybit_client.get_available_balance()
            if total_account_balance <= 1.0: # Check for a minimum of $1
                logger.error(f"[{trade_id}] Insufficient account balance to allocate. Total balance: ${total_account_balance:.2f}. Aborting trade.")
                return

            # Calculate allocation per position: use ~95% of account divided by max concurrent symbols
            # This ensures each position gets an equal share of the available account
            allocation_percentage = 0.95  # Use 95% of account, reserve 5% for fees/buffer
            allocated_balance_for_symbol = (total_account_balance * allocation_percentage) / self.max_concurrent_symbols

            if allocated_balance_for_symbol <= 0:
                logger.error(f"[{trade_id}] Calculated allocated balance per position is zero or negative. Aborting trade.")
                return

            logger.info(f"[{trade_id}] Position allocation: Total balance=${total_account_balance:.2f}, "
                       f"Max symbols={self.max_concurrent_symbols}, "
                       f"Allocation per position=${allocated_balance_for_symbol:.2f}")
            ...
            logger.info(f"[{trade_id}] Dynamic allocation: Total account balance=${total_account_balance:.2f}, "
                       f"Max symbols={self.max_concurrent_symbols}, "
            ...
            # Pass the specific allocated amount for this symbol to the quantity calculator
    ...
    async def guardian_loop(self):
        ...
        # Add periodic position sync every 5 cycles (about every 5-10 seconds depending on loop interval)
        position_sync_counter = 0
        POSITION_SYNC_INTERVAL = 5

        while self.is_running:
            try:
                # Periodically sync with Bybit to ensure accurate position count
    ...
    def _calculate_arsenal_style_sl(self, direction: str, current_price: float, hotspots: list) -> float:
        """Calculates a fixed stop loss to prevent abrupt stop outs from liquidity-based stops."""
        # Fixed stop loss at 0.3% from entry regardless of liquidity analysis
        # This prevents the system from setting very tight stops based on liquidity that get triggered too quickly
        sl_distance = current_price * 0.003  # 0.3%

        if direction == 'LONG':
            # For long positions, stop loss is below entry price
            return current_price * 0.997  # 0.3% below current price
        else: # SHORT
            # For short positions, stop loss is above entry price
            return current_price * 1.003  # 0.3% above current price
    ...
    async def run_aegis_analysis(self, session: dict, market_data: dict, smoothed_price: float) -> tuple[str, str]:
        ...
        # --- Standard Procedure Checks (Breakeven) ---
        # This check is now independent of the score
        if not trade.sl_moved_to_breakeven:
            distance_to_tp = abs(trade.take_profit - trade.entry_price)
            progress_to_tp = abs(smoothed_price - trade.entry_price)
            if distance_to_tp > 0 and (progress_to_tp / distance_to_tp) > 0.75:
                return "MOVE_SL_BREAKEVEN", "Price has achieved 75% of distance to TP"
            ...
            async with self.pending_orders_lock:
                ...
                for order_id, order_details in pending_orders_copy:
                    try:
                        ...
                        time_elapsed = get_utc_now() - order_time
