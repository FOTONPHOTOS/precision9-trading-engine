...
class smc:
    ...
    def sessions(
        ...
        """
        Sessions
        This method returns which candles are within the session specified, based on UTC.

        parameters:
        session_name: str - the session you want to check (ASIA, LONDON, NEW_YORK)
        ...
        """
        from timezone_utils import SESSIONS # Import our centralized session definitions

        if session_name.upper() not in SESSIONS:
            raise ValueError(f"Session '{session_name}' not found. Available sessions are: {list(SESSIONS.keys())}")

        session = SESSIONS[session_name.upper()]
        start_time = session["start"]
        end_time = session["end"]

        # Ensure ohlc index is timezone-aware and in UTC
        if ohlc.index.tzinfo is None:
        ...
        for i in range(len(ohlc)):
            current_time = ohlc.index[i].time()
            
            is_active = False
            # Handle overnight sessions like ASIA that cross midnight
            if start_time > end_time:
                if current_time >= start_time or current_time < end_time:
                    is_active = True
            else: # Handle normal same-day sessions
